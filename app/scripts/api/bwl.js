...
Date.prototype.getGMTOffset = function() {
    return (this.getTimezoneOffset() > 0 ? "-" : "+")
            + BWL.leftPad(Math.abs(Math.floor(this.getTimezoneOffset() / 60)),
                    2, '0')
...
var BWL = {
    ...
    InvokeService : function(method, url, data, successCallback, errorCallback) {
        ...
        if (method == "POST" || method == "PUT") {
            ...
            // if this is a JS object, convert to JSON, else just pass the data
            // as form data
        ...
        xhd.request(requestConfig,
                ...
                function(rpcdata) {
                    ...
                    if (modelObj.Message == 'OK') {
                        ...
                        successCallback(BWL.Plugins.json2
                                .parse(modelObj.Object), modelObj);
                    } else {
            ...
            for ( var item in arr) {
                ...
                if (typeof value == 'object') { // If it is an array,
                    ...
                    if (value != null && value != undefined) {
                        if (eolchar == "\n") {
                            dumped_text += BWL.dump(value, eolchar, level + 1,
                                    levelId + ".");
                        } else {
                            var label = "";

                            // add the type, key or both to the label if
                            // available
                            ...
                            dumped_text += " <a href=\"javascript:BWL.toggle('"
                                    + levelId + "');\">" + label + "</a>";
                            dumped_text += eolchar;
                            dumped_text += "<div id='" + levelId
                                    + "' style='display: none'>";
                            dumped_text += BWL.dump(value, eolchar, level + 1,
                                    levelId + ".");
                            dumped_text += "</div>";
...
BWL.oAuth = {
    ...
    LogonAsync : function(provider, successCallback, errorCallback) {
        ...
        BWL.oAuth.PopupWindow = BWL.Helpers.UI.PopupCenter(authURL,
                'AuthWindow', BWL.oAuth.PopupWidth, BWL.oAuth.PopupHeight);
    ...
    AddLogonAsync : function(provider, successCallback, errorCallback) {
        ...
        if (BWL.Helpers.UI.IsPopupClosed(BWL.oAuth.PopupWindow)) {
            ...
            window.location = this
                    .GetAuthAddLogonURL(provider, window.location);
        } else {
            // now start watching the login window
            setTimeout(BWL.oAuth.CheckLoginWindow, BWL.RETRYDELAY);
        }
...
BWL.Membership = {
    RegisterAsync : function(fullName, email, password, successCallback,
        ...
        if (successCallback == undefined) {
            successCallback = this.RegisterCallback;
        }
        if (errorCallback == undefined) {
            errorCallback = BWL.ServiceFailedCallback;
        }
    ...
    ResetPasswordAsync : function(email, successCallback, errorCallback) {
        ...
        if (successCallback == undefined) {
            successCallback = this.ResetPasswordCallback;
        }
        if (errorCallback == undefined) {
            errorCallback = BWL.ServiceFailedCallback;
        }
        ...
        BWL.Services.MembershipService.ResetPasswordAsync(BWL.StoreKey,
                membership, successCallback, errorCallback);
    },
    ...
    ChangePasswordAsync : function(email, currentPassword, newPassword,
        ...
        if (successCallback == undefined) {
            successCallback = this.ChangePasswordCallback;
        }
        if (errorCallback == undefined) {
            errorCallback = BWL.ServiceFailedCallback;
        }
        ...
        BWL.Services.MembershipService.ChangePasswordAsync(BWL.StoreKey,
                BWL.oAuth.HashPassword(newPassword), membership,
                successCallback, errorCallback);
    },
...
BWL.Helpers.UI = {
    Inject : function(template, item, crawlitem) {
        if (crawlitem == undefined || crawlitem == null) {
            crawlitem = true;
        }
        ...
        if (item != undefined && item != null) {
            ...
            for (prop in item) {
                ...
                if (item[prop] != null && typeof item[prop] == "object") {
                    if (crawlitem) {
                        template = this.InjectItem(template, item[prop],
                                crawlitem, prefix + prop + ".");
                    }
                } else {
                    template = template.replace(new RegExp('\\{item.' + prefix
                            + prop + '\\}', 'gi'), item[prop]);
                }
            }
        }
        ...
        if (data != undefined && data != null) {
            ...
            for (pos in data) {
                items += selected == data[pos].Key ? this.Inject(BWL.$(
                        elementName + "_view_selected").html(), data[pos])
                        : this.Inject(BWL.$(elementName + "_view_item").html(),
                                data[pos]);
            }
        }
        ...
        if (data != undefined && data != null) {
            ...
            for (pos in data) {
                ...
                if (rowCount > itemsperrow) {
                    ...
                    rowCount = 1;
                    // add the row
                    rows += BWL.$(elementName + "_view_row").html().replace(
                            "{items}", items);
                    items = "";
                }
                // add the item
    ...
    IsPopupClosed : function(popup) {
        ...
        try {
            windowClosed = !popup || popup.closed
                    ...
                    || typeof popup.document.getElementById == "undefined";
        } catch (error) {
        }
        return windowClosed;
    },
...
BWL.Helpers.Category = {
    SearchForCategoryItems : function(searchObj, categoryReturn) {
        if (searchObj != undefined && searchObj != null) {
            if (BWL.isArray(searchObj)) {
                ...
                if (searchObj.Type != 'Store'
                        && searchObj.Categories != undefined
                    ...
                    for ( var prop in searchObj) {
                        if (searchObj[prop] != undefined
                                && searchObj[prop] != null) {
                            if (typeof searchObj[prop] == 'object') {
                                this.SearchForCategoryItems(searchObj[prop],
                                        categoryReturn);
                            }
                        }
                    }
    ...
    GetCategoriesWithItems : function() {
        ...
        // for caregories, crawl an object looking for Items or Events lists,
        // and then in them, they will have a Categories item, search that
    ...
    GetCategoryWithItems : function(categoryKey) {
        if (BWL.Store._categoryCache == undefined) {
            BWL.Store._categoryCache = BWL.Helpers.Category
            ...
            if (category.ChildCategories != null
                ...
                for ( var childPos in category.ChildCategories) {
                    return this.CheckCategory(
                            category.ChildCategories[childPos], categoryKey);
                }
            }
        }
        return null;
    ...
    FindStoreCategory : function(categoryKey) {
        ...
        if (BWL.Store != null && BWL.Store.Categories != undefined
                && BWL.Store.Categories != null
                && BWL.isArray(BWL.Store.Categories)) {
