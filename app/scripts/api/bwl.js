...
Date.prototype.getGMTOffset = function () {
    return (this.getTimezoneOffset() > 0 ? "-" : "+")
        + BWL.leftPad(Math.abs(Math.floor(this.getTimezoneOffset() / 60)), 2, '0')
                ...
                if (typeof (value) == 'object') { //If it is an array,
                    ...
                    if (value != null && value != undefined) {
                        if (eolchar == "\n") {
                            ...
                            dumped_text += " <a href=\"javascript:BWL.toggle('" + levelId + "');\">" + label + "</a>";
                            dumped_text += eolchar;
...
BWL.oAuth = {
    ...
    LoadProfileAsync: function (successCallback, errorCallback) {
        ...
        BWL.Services.SystemProfileService.GetProfileAsync(5, BWL.oAuth.GetProfileCallback);
    },
    ...
    GetAuthLogonURL: function (providerName, redirectURL) {
        return BWL.Server + '/oAuth/logon?providerName={0}&clientKey={1}&display=HTML&redirectURL={2}'.format(providerName, BWL.oAuth.ClientKey, redirectURL);
    },

    GetAuthAddLogonURL: function (providerName, redirectURL) {
        return BWL.Server + '/oAuth/addLogon?providerName={0}&clientKey={1}&display=HTML&redirectURL={2}'.format(providerName, BWL.oAuth.ClientKey, redirectURL);
    },
    ...
    LogonAsync: function (provider, successCallback, errorCallback) {
        ...
        BWL.oAuth.PopupWindow = BWL.Helpers.UI.PopupCenter(authURL, 'AuthWindow', BWL.oAuth.PopupWidth, BWL.oAuth.PopupHeight);
...
BWL.Membership = {
    RegisterAsync: function (fullName, email, password, successCallback, errorCallback) {
        ...
        BWL.Services.MembershipService.RegisterAsync(BWL.StoreKey, membership, successCallback, errorCallback);
    },
    ...
    LoginAsync: function (email, password, successCallback, errorCallback) {
        ...
        BWL.Services.MembershipService.LoginAsync(BWL.StoreKey, membership, successCallback, errorCallback);
    },
    ...
    ResetPasswordAsync: function (email, successCallback, errorCallback) {
        ...
        BWL.Services.MembershipService.ResetPasswordAsync(BWL.StoreKey, membership, successCallback, errorCallback);
    },
    ...
    ChangePasswordAsync: function (email, currentPassword, newPassword, successCallback, errorCallback) {
        ...
        BWL.Services.MembershipService.ChangePasswordAsync(BWL.StoreKey, BWL.oAuth.HashPassword(newPassword), membership, successCallback, errorCallback);
    },
...
BWL.Helpers.UI = {
        ...
        if (item != undefined && item != null) {
            ...
            for (prop in item) {
                ...
                if (item[prop] != null && typeof (item[prop]) == "object") {
                    if (crawlitem)
                        template = this.InjectItem(template, item[prop], crawlitem, prefix + prop + ".");
                } else {
                    template = template.replace(new RegExp('\\{item.' + prefix + prop + '\\}', 'gi'), item[prop]);
                }
            }
        }
    ...
    SortDisplayOrder: function (data) {
        ...
        if (data != undefined && data.length > 0 && data[0].DisplayOrder != undefined) {
    ...
    BuildListTemplate: function (elementName, data, selected) {
        ...
        if (data != undefined && data != null) {
            ...
            for (pos in data) {
                items += (selected == data[pos].Key) ? this.Inject(BWL.$(elementName + "_view_selected").html(), data[pos]) : this.Inject(BWL.$(elementName + "_view_item").html(), data[pos]);
            }
        }
        ...
        if (data != undefined && data != null) {
            ...
            for (pos in data) {
                ...
                if (rowCount > itemsperrow) {
                    ...
                    rowCount = 1;
                    // add the row
                    rows += BWL.$(elementName + "_view_row").html().replace("{items}", items);
                    items = "";
                }
                // add the item
                items += this.Inject(BWL.$(elementName + "_view_item").html(), data[pos]);
            }
    ...
    IsPopupClosed: function (popup) {
        ...
        try {
            windowClosed = (!popup || popup.closed || typeof popup.closed == 'undefined' || typeof popup.document.getElementById == "undefined");
        }
        catch (error) { }
        return windowClosed;
    },
    ...
    PopupCenter: function (url, title, width, height) {
        ...
        var popup = window.open(url, title, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=no, width=' + width + ', height=' + height + ', top=' + top + ', left=' + left);
    ...
    OpenInlineBrowser: function (url, title, width, height) {
        ...
        var browser = BWL.$('<div><input type="text" style="width:100%;" disabled="disabled" value="' + url + '" /><br/><iframe src="' + url + '" style="width:100%;height:' + height + 'px;" /></div>')
            .dialog({
                title: title,
                bgiframe: true,
                autoOpen: false,
    ...
    OpenDialog: function (html, title, width, height) {
        ...
        if (this.DialogCache[id] == undefined || this.DialogCache[id] == null) {
            // create
            this.DialogCache[id] = BWL.$('<div></div>')
                .html(html)
                .dialog({
                    title: title,
                    bgiframe: true,
                    ...
                    draggable: true,
                    resizable: true,
                    close: function () { BWL.Helpers.UI.CloseDialogCallback(id); }
                });
        }
...
BWL.Helpers.Category = {
    SearchForCategoryItems: function (searchObj, categoryReturn) {
        if (searchObj != undefined && searchObj != null) {
            if (BWL.isArray(searchObj)) {
                ...
                if (searchObj.Type != 'Store' && searchObj.Categories != undefined && BWL.isArray(searchObj.Categories)) {
                    ...
                    for (var categoryPos in searchObj.Categories) {
                        ...
                        if (!BWL.isArray(categoryReturn[category.Key]))
                            categoryReturn[category.Key] = new Array();
    ...
    GetCategoriesWithItems: function () {
        ...
        // for caregories, crawl an object looking for Items or Events lists, and then in them, they will have a Categories item, search that
    ...
    GetCategoryWithItems: function (categoryKey) {
        ...
        return (BWL.Store._categoryCache[categoryKey] != undefined) ? BWL.Store._categoryCache[categoryKey] : null;
    },
    ...
    CheckCategory: function (category, categoryKey) {
        if (category != null) {
            ...
            if (category.ChildCategories != null && category.ChildCategories != undefined && BWL.isArray(category.ChildCategories)) {
                for (var childPos in category.ChildCategories) {
                    return this.CheckCategory(category.ChildCategories[childPos], categoryKey);
                }
            }
        }
        return null;
    ...
    FindStoreCategory: function (categoryKey) {
        ...
        if (BWL.Store != null && BWL.Store.Categories != undefined && BWL.Store.Categories != null && BWL.isArray(BWL.Store.Categories)) {
            for (var catPos in BWL.Store.Categories) {
                var category = BWL.Helpers.Category.CheckCategory(BWL.Store.Categories[catPos], categoryKey);
                if (category != null)
                    return category;
            }
        }
